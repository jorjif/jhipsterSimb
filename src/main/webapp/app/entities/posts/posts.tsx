              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('img')}>
                    <Translate contentKey="jhipsterApp.posts.img">Img</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('img')} />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {postsList.map((posts, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{posts.title}</td>
                    <td>{posts.content}</td>
                    <td>{posts.date ? <TextFormat type="date" value={posts.date} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{posts.views}</td>
                    <td>
                      {posts.img ? (
                        <div>
                          {posts.imgContentType ? (
                            <a onClick={openFile(posts.imgContentType, posts.img)}>
                              <img src={`data:${posts.imgContentType};base64,${posts.img}`} style={{ maxHeight: '30px' }} />
                              &nbsp;
                            </a>
                          ) : null}
                          <span>
                            {posts.imgContentType}, {byteSize(posts.img)}
                          </span>
                        </div>
                      ) : null}
                    </td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button tag={Link} to={`/posts/${posts.id}/delete`} color="danger" size="sm" data-cy="entityDeleteButton">
