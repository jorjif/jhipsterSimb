...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "select post from Post post left join fetch post.post", countQuery = "select count(post) from Post post")
    Page<Post> findAllWithToOneRelationships(Pageable pageable);

    @Query("select post from Post post left join fetch post.post")
    List<Post> findAllWithToOneRelationships();

    @Query("select post from Post post left join fetch post.post where post.id =:id")
